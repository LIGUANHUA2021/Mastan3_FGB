...
from analysis.FESect.util import FindComponetMaxMinCoord
from analysis.FESect.util import FindStress
from analysis.FESect.util import CalMaxAxialForces
from analysis.FESect.util import CalMinAxialForces
##
from analysis.FESect.variables.Result import SectionProperties as SP
from analysis.FESect.solver import BasicProperties as BP
#
...
class CalYieldS:
    ...
    def Initialize(self):
        ...
        SP.Area = sum(
            [Model.Material.E[Model.Fiber.MaterialID[i]] * Model.Fiber.Area[i] for i in Model.Fiber.ID]) / E_ref
        ...
        SP.cy = SP.Qz / SP.Area
        SP.cz = SP.Qy / SP.Area
        (SP.Iyc, SP.Izc) = BP.GetIyIz(Model.Fiber.ID, Model.Fiber.Area, Model.Fiber.Iy, Model.Fiber.Iz,
                                      Model.Fiber.cy, Model.Fiber.cz, SP.cy, SP.cz, E_ref)
        SP.Iyzc = BP.GetIyz(Model.Fiber.ID, Model.Fiber.Area, Model.Fiber.Iyz,
                            Model.Fiber.cy, Model.Fiber.cz, SP.cy, SP.cz, E_ref)
        if SP.Iyc == SP.Izc:
            SP.Theta = 0
        elif SP.Iyc < SP.Izc:
        ...
        # print("Ygc = ", Ygc)
        # print("Zgc = ", Zgc)
        for ii in Model.Fiber.ID:
            YCoor = Model.Fiber.cy[ii]
            ZCoor = Model.Fiber.cz[ii]
            Model.Fiber.cygo[ii] = YCoor - Ygc
            Model.Fiber.czgo[ii] = ZCoor - Zgc
        ### For testing
        # df1 = pd.DataFrame.from_dict(list(Model.Fiber.cy.values()))
        # df2 = pd.DataFrame.from_dict(list(Model.Fiber.cz.values()))
        # filepath1 = "C:/Users/gaowl/PycharmProjects/Mastan3/Source/analysis/FESect/FESectFiberTest_cy.xlsx"
        # with pd.ExcelWriter(filepath1) as writer:
        #     # 将 DataFrame 写入 Excel 文件
        #     df1.to_excel(writer, sheet_name='Sheet1', index=False)
        # filepath2 = "C:/Users/gaowl/PycharmProjects/Mastan3/Source/analysis/FESect/FESectFiberTest_cz.xlsx"
    ...
    def CalSecCapacity(self, InNx, InAngle, MaxAxial, MinAxial):
        ...
        if Model.YieldSurfaceAnalInfo.BStrainControl == 0:
            for j in Model.Material.ID:
                Model.Material.MaxComStrn[j] = Model.Material.eu[j]
                Model.Material.MaxTenStrn[j] = -Model.Material.eu[j]
        ...
        # pl.Print(" >>>  (CalSectCapacity)Model.YieldSurfaceAnalInfo.BStrainControl ="+f"{Model.YieldSurfaceAnalInfo.BStrainControl}")
        for ii in Model.Material.ID:
            ## Maximum Compressive Strain and Location
            # MaxV = OptCoordbyComp[ii]['MaxV']
            MaxV = Model.Fiber.MaxV
            # print("Test MaxV = ", MaxV)
            # print("Test tMaxV = ", tMaxV)
            if MaxV > 0:
                TempComStr = Model.Material.MaxComStrn[ii]
                # print(">>> Cal ComStrn = ", TempComStr)
            ...
            # print("Test MinV = ", MinV)
            # print("Test tMinV = ", tMinV)
            if MinV < 0:
                TempTenStr = Model.Material.MaxTenStrn[ii]
                # print(">>> Cal TenStrn = ", TempTenStr)
        ...
        PreN = abs(InNx / 100000)
        if PreN == 0.0: PreN = 0.001
        Idn = Idn_limt
    ...
    def RotateToAngle(self, tdA):
        ...
        ##
        for ii in Model.Fiber.ID:
            YCoor = Model.Fiber.cygo[ii]
            ZCoor = Model.Fiber.czgo[ii]
            Model.Fiber.cv[ii] = -YCoor*math.cos(tdA) + ZCoor*math.sin(tdA)
            Model.Fiber.cw[ii] = YCoor*math.sin(tdA) + ZCoor*math.cos(tdA)
            tV = Model.Fiber.cv[ii]
            tW = Model.Fiber.cw[ii]
    ...
    def CalFullYSurfacePMyMz(self, AxisSlctn):
        ...
        if AxisSlctn == 2:
            for ii in Model.Fiber.ID:
                YCoor = Model.Fiber.cygo[ii]
                ZCoor = Model.Fiber.czgo[ii]
                Model.Fiber.cygo[ii] = -YCoor * math.cos(Phi) + ZCoor * math.sin(Phi)
                Model.Fiber.czgo[ii] = YCoor * math.sin(Phi) + ZCoor * math.cos(Phi)
        ...
        for ii in np.arange(PosNStep):
            ...
            if IsConvergence:
                ...
                pl.Print(" >>>  Analysis under axial load = ".ljust(30,' ') + str(str("%.3f" % (InNx)) + ", Convergence = " + str(bool(IsConvergence))).rjust(48,' '))
                #
        ##========================================================================================
    ...
    def CalPlanarYSurfaceMyMz(self, AxisSlctn):
        ...
        # pl.Print("Cal:Model.YieldSurfaceAnalInfo.BStrainControl =" + f"{Model.YieldSurfaceAnalInfo.BStrainControl}")
        # pl.Print("Cal:Model.YieldSAnalResults.MyMz_StrnContlType =" + f"{Model.YieldSAnalResults.MyMz_StrnContlType}")
