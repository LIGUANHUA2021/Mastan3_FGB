...
def OutCyCRes(result):
    ...
    for ii in Model.Node.ID:
        ...
        RowRes[1] = ii
        RowRes[2] = result.CyCRes.CurU[tvID * 6]
        RowRes[3] = result.CyCRes.CurU[tvID * 6 + 1]
        RowRes[4] = result.CyCRes.CurU[tvID * 6 + 2]
        RowRes[5] = result.CyCRes.CurU[tvID * 6 + 3]
        RowRes[6] = result.CyCRes.CurU[tvID * 6 + 4]
        RowRes[7] = result.CyCRes.CurU[tvID * 6 + 5]
        AddCSVRow(fileName, RowRes)
    ##
    fileName = ResultFolder + os.sep + Model.OutResult.ModelName + '-Bound.csv'
    ...
    for ii in Model.Element.ID:
        ...
        RowRes[5] = tElePStrs_list[3]
        RowRes[6] = tElePStrs_list[4]
        RowRes[7] = tElePStrs_list[5]
        AddCSVRow(fileName, RowRes)
    return
