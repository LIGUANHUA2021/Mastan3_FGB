...
def GetEleKL(L,E,G,Sect,tSID):
    ...
    tMtx[6, 6] = (120.0 * E * Cw * L + 4.0 * G * J * L ** 3) / (30.0 * L ** 2);
    tMtx[7, 7] = E * A / L
...
def GetEleKG(L,P,My1,My2,Mz1,Mz2,Mx2,Sect,tSID):
    ...
    # Non-diagonal
    tMtx[0, 7] = -1.0 * P / L
    tMtx[1, 3] = (11.0 * My1 - My2) / (10.0 * L)
    tMtx[1, 4] = Mx2 / L
    tMtx[1, 5] = P / 10.0
    tMtx[1, 8] = -6.0 * P / (5.0 * L)
    tMtx[1, 10] = -1.0 * (My1 - 11.0 * My2) / (10.0 * L)
    tMtx[1, 11] = -1.0 * Mx2 / L
    tMtx[1, 12] = P / 10.0
    tMtx[1, 6] = My1 / 10.0
    tMtx[1, 13] = -1.0 * My2 / 10.0
    tMtx[2, 3] = (11.0 * Mz1 - Mz2) / (10.0 * L)
    tMtx[2, 4] = -1.0 * P / 10.0
    tMtx[2, 5] = Mx2 / L
    tMtx[2, 9] = -6.0 * P / (5.0 * L)
    tMtx[2, 10] = -1.0 * (Mz1 - 11.0 * Mz2) / (10.0 * L)
    tMtx[2, 11] = -1.0 * P / 10.0
    tMtx[2, 12] = -1.0 * Mx2 / L
    tMtx[2, 6] = Mz1 / 10.0
    tMtx[2, 13] = -1.0 * Mz2 / 10.0
    tMtx[3, 8] = -1.0 * (11.0 * My1 - My2) / (10.0 * L)
    tMtx[3, 9] = -1.0 * (11.0 * Mz1 - Mz2) / (10.0 * L)
    tMtx[3, 10] = -1.0 * 6.0 * P * Ip / (5.0 * A)
    tMtx[3, 11] = -1.0 * (2.0 * Mz1 + Mz2) / 10.0
    tMtx[3, 12] = (2.0 * My1 + My2) / 10.0
    tMtx[3, 6] = P * Ip / (10.0 * A)
    tMtx[3, 13] = P * Ip / (10.0 * A)
    tMtx[4, 8] = -1.0 * Mx2 / L
    tMtx[4, 9] = P / 10.0
    tMtx[4, 10] = -1.0 * (Mz1 + 2.0 * Mz2) / 10.0
    tMtx[4, 11] = -1.0 * P * L / 30.0
    tMtx[4, 12] = Mx2 / 2.0
    tMtx[4, 6] = -1.0 * (3.0 * Mz1 - Mz2) * L / 30.0
    tMtx[4, 13] = Mz1 * L / 30.0
    tMtx[5, 8] = -1.0 * P / 10.0
    tMtx[5, 9] = -1.0 * Mx2 / L
    tMtx[5, 10] = (My1 + 2.0 * My2) / 10.0
    tMtx[5, 11] = -1.0 * Mx2 / 2.0
    tMtx[5, 12] = -1.0 * P * L / 30.0
    tMtx[5, 6] = (3.0 * My1 - My2) * L / 30.0
    tMtx[5, 13] = -1.0 * My1 * L / 30.0
    tMtx[8, 10] = (My1 - 11.0 * My2) / (10.0 * L)
    tMtx[8, 11] = Mx2 / L
    tMtx[8, 12] = -1.0 * P / 10.0
    tMtx[8, 6] = -1.0 * My1 / 10.0
    tMtx[8, 13] = My2 / 10.0
    tMtx[9, 10] = (Mz1 - 11.0 * Mz2) / (10.0 * L)
    tMtx[9, 11] = P / 10.0
    tMtx[9, 12] = Mx2 / L
    tMtx[9, 6] = -1.0 * Mz1 / 10.0
    tMtx[9, 13] = Mz2 / 10.0
    tMtx[10, 12] = -1.0 * (My1 - 2.0 * My2) / 5.0
    tMtx[10, 6] = -1.0 * P * Ip / (10.0 * A)
    tMtx[10, 13] = -1.0 * P * Ip / (10.0 * A)
    tMtx[11, 6] = -1.0 * Mz2 * L / 30.0
    tMtx[11, 13] = -1.0 * (Mz1 - 3.0 * Mz2) * L / 30.0
    tMtx[12, 6] = My2 * L / 30.0
    tMtx[12, 13] = (My1 - 3.0 * My2) * L / 30.0
    tMtx[6, 13] = -P * Ip * L / (30.0 * A)
    # Old factors
    ...
    # Diagonal
    tMtx[0, 0] = P / L
    tMtx[1, 1] = 6.0 * P / (5.0 * L)
    tMtx[2, 2] = 6.0 * P / (5.0 * L)
    tMtx[3, 3] = 6.0 * P * Ip / (5.0 * A * L)
    tMtx[4, 4] = 2.0 * P * L / 15.0
    tMtx[5, 5] = 2.0 * P * L / 15.0
    tMtx[7, 7] = P / L
    tMtx[8, 8] = 6.0 * P / (5.0 * L)
    tMtx[9, 9] = 6.0 * P / (5.0 * L)
    tMtx[10, 10] = 6.0 * P * Ip / (5.0 * A * L)
    tMtx[11, 11] = 2.0 * P * L / 15.0
    tMtx[12, 12] = 2.0 * P * L / 15.0
    tMtx[6, 6] = 2.0 * P * Ip / (15.0 * A)
