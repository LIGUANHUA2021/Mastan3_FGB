...
from gui.msasect.ui.Ui_HollowPolyDb import Ui_HollowPolyDb_Dialog

class library_Path:
    Path = r"base/library/"
    SectType = r"HollowPoly"


class HollowPolyDb_Dialog(QDialog, Ui_HollowPolyDb_Dialog):
    """
    ...
    def on_OK_button_clicked(self):
        ...
        if self.ComboBox.currentText()=="User-Polygon(in)":
            unit=0
        else:
            unit = 1
    ...
    def initlize(self):
        self.label.setPixmap(QPixmap("ui/Template/Hollow Poly_Db.jpg"))
        Database = self.ComboBox.currentText()
        if Database == "User-Polygon(in)":
    ...
    def load_DimensionsTable(self, index):
        ...
        self.Dimensions_tableWidget.setRowCount(len(Section) - 1)
