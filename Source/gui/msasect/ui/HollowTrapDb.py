...
from gui.msasect.ui.Ui_HollowTrapDb import Ui_HollowTrapDb_Dialog

class library_Path:
    Path = r"base/library/"
    SectType = r"User-Trap"


class HollowTrapDb_Dialog(QDialog, Ui_HollowTrapDb_Dialog):
    """
    ...
    def on_OK_button_clicked(self):
        ...
        if self.ComboBox.currentText()=="User-Trap(in)":
            unit=0
        else:
            unit = 1
    ...
    def initlize(self):
        self.label.setPixmap(QPixmap("ui/Template/Hollow Trap_Db.jpg"))
        Database = self.ComboBox.currentText()
        if Database == "User-Trap(in)":
