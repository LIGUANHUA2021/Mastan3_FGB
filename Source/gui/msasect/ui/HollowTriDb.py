...
from gui.msasect.ui.Ui_HollowTriDb import Ui_HollowTriDb_Dialog

class library_Path:
    Path = r"base/library/"
    SectType = r"HollowTri"


class HollowTriDb_Dialog(QDialog, Ui_HollowTriDb_Dialog):
    """
    ...
    def on_OK_button_clicked(self):
        ...
        if self.ComboBox.currentText()=="User-Triangle(in)":
            unit=0
        else:
            unit = 1
    ...
    def initlize(self):
        self.label.setPixmap(QPixmap("ui/Template/Hollow Tri_Db.jpg"))
        Database = self.ComboBox.currentText()
        if Database == "User-Triangle(in)":
