...
class Model3DViewDialog(QDialog, Ui_Model3DView_Dialog):
    """
    ...
    def __init__(self, parent=None):
        ...
        self.ClChecked = parent.Centerline_radioButton.isChecked()
        self.Ext = 0
        self.Transparency = 1
    ...
    def Model3DPlot(self):
        ...
        if self.Volume_checkBox.isChecked():
            if self.ClChecked:
                verts1 = np.full([msaModel.Segment.Count * 4, 3], self.Ext * 2)
                ...
                verts1 = np.full([TempNode.Count, 3], self.Ext * 2)
                ...
                Back.setGLOptions('translucent')
                self.graphicsView.addItem(Back)
                vertsf = np.full([msaFEModel.Point.Count, 3], self.Ext * 2)
                vertsb = np.full([msaFEModel.Point.Count, 3], self.Ext * -2)
        ...
        if self.Edge_checkBox.isChecked():
            if self.ClChecked:
                ...
                for i in msaFEModel.Loop.ID:
                    pts = np.zeros((len(msaFEModel.Loop.PointID[i]) + 1, 3))
                    for j in range(len(msaFEModel.Loop.PointID[i])):
                        pts[j, 1] = msaFEModel.Point.Zo[msaFEModel.Loop.PointID[i][j]]
                        pts[j, 2] = msaFEModel.Point.Yo[msaFEModel.Loop.PointID[i][j]]
                        pts[j, 0] = self.Ext * 2
                    pts[-1, 1] = pts[0, 1]
                    pts[-1, 2] = pts[0, 2]
                    pts[-1, 0] = self.Ext * 2
                ...
                for i in msaFEModel.Point.ID:
                    pts = np.array([[self.Ext * 2, msaFEModel.Point.Zo[i], msaFEModel.Point.Yo[i]],
                                    [self.Ext * -2, msaFEModel.Point.Zo[i], msaFEModel.Point.Yo[i]]])
                    plt3 = gl.GLLinePlotItem(pos=pts, color=(0.8, 0.8, 0.8, self.Transparency), width=1, antialias=True)
                    self.graphicsView.addItem(plt3)
