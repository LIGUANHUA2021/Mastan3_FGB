...
from .Ui_MCAnalMessageBox import Ui_MCAnalMessageBox_Dialog
...
class MCAnalMessageBox_Dialog(QDialog, Ui_MCAnalMessageBox_Dialog):
    """
    Class documentation goes here.
    """
    MCCalProgress_Signal = Signal(int)
    MCCalFinish_Signal = Signal()
    ...
    def __init__(self, mw, parent):
        ...
        self.MCCalFinish_Signal.connect(self.MomentCurvaCalFinish)
        ##
        if self.grandparent_window.Centerline_radioButton.isChecked():
            self.NewThread = MultiThread(label="CalMC",
                                         parameter={"mw": self, "AnaFileName": msaModel.Information.ModelName, "Flag": 1,
                                                    "progress_Signal": self.MCCalProgress_Signal,
                                                    "finish_Signal": self.MCCalFinish_Signal})
        else:
            self.NewThread = MultiThread(label="CalMC",
    ...
    def on_OK_pushButton_clicked(self):
        ...
        # raise NotImplementedError
        QDialog.close(self)
        Ui = ShowResultsMCurv_Dialog(parent=self.mw)
        Ui.exec()

    def MomentCurvaCalFinish(self):
